...
export default function Home() {
  ...
  return (
    <main className="app">
      ...
      {cartOpen && <CartDrawer items={cartItems} cart={cart} subtotal={subtotal} delivery={delivery} updateCart={updateCart} close={() => setCartOpen(false)} />}
    </main>
  );
}
...
function ProductCard({ product, addToCart }: { product: Product; addToCart: (id: number) => void }) {
  return <article className="product-card"><div className="product-image" style={{ backgroundColor: product.color }}><img src={product.image} alt={product.name} />{product.badge && <span className="badge">{product.badge}</span>}<button className="quick-add" onClick={() => addToCart(product.id)}>+</button></div><div className="product-info"><div><h3>{product.name}</h3><p>{product.unit}</p></div><div className="price-row"><strong>{peso(product.price)}</strong>{product.oldPrice && <del>{peso(product.oldPrice)}</del>}<button onClick={() => addToCart(product.id)}>Add</button></div></div></article>;
}

function CartDrawer({ items, cart, subtotal, delivery, updateCart, close }: { items: Product[]; cart: Record<number, number>; subtotal: number; delivery: number; updateCart: (id: number, change: number) => void; close: () => void }) {
  return <div className="overlay" onClick={close}><aside className="cart-drawer" onClick={(event) => event.stopPropagation()}><div className="drawer-head"><div><span className="kicker">Your basket</span><h2>{Object.values(cart).reduce((a, b) => a + b, 0)} items</h2></div><button onClick={close}>×</button></div>{items.length === 0 ? <div className="cart-empty"><span>🧺</span><h3>Your basket is waiting</h3><p>Add something fresh from the market.</p></div> : <><div className="cart-items">{items.map((item) => <div className="cart-item" key={item.id}><img src={item.image} alt="" /><div><strong>{item.name}</strong><small>{item.unit}</small><b>{peso(item.price)}</b></div><div className="stepper"><button onClick={() => updateCart(item.id, -1)}>−</button><span>{cart[item.id]}</span><button onClick={() => updateCart(item.id, 1)}>+</button></div></div>)}</div><div className="cart-summary"><div><span>Subtotal</span><b>{peso(subtotal)}</b></div><div><span>Delivery</span><b>{delivery === 0 ? "Free" : peso(delivery)}</b></div><div className="total"><span>Total</span><b>{peso(subtotal + delivery)}</b></div><button className="primary checkout">Continue to checkout <span>→</span></button><small>🔒 Secure checkout · Pay with card, GCash, or cash</small></div></>}</aside></div>;
}
